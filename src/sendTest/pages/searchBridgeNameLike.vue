<template>
  <div>
    <input type="text" v-model="bridgename" placeholder="请输入桥梁关键字"/>
    <button @click="getBridge">确定</button>
      ...
      <el-table-column fixed="right" label="操作" width="200">
        <template slot-scope="scope">
          ...
          <el-button
            ...
            size="small"
            circle="true"
            @click="showBridgeScore(scope.row.id,scope.row.name)"
            >查看得分</el-button
          >
          <el-button
            type="text"
...
export default {
  ...
  methods: {
    ...
    getBridge() {
      axios
        .get(
          `http://localhost:8080/bridge/nameLike?bridgeName=${this.bridgename}`
        )
        .then(
          (response) => {
